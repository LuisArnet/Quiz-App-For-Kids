...
#import "QuizEightViewController.h"

@interface QuizEightViewController (){
...
@implementation QuizEightViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    order=0;
    problemArray=[[NSArray alloc] initWithObjects:@"2*2-3*3",@"4*2-5*2",@"2*3-3*4",@"2*3-3*2",@"3*2-2*6",nil];
    firstArray=[[NSArray alloc] initWithObjects:@"-3",@"-3",@"-6",@"-1",@"-5", nil];
    secondArray=[[NSArray alloc] initWithObjects:@"-4",@"-2",@"-5",@"0",@"-6", nil];
    thirdArray=[[NSArray alloc] initWithObjects:@"-5",@"-1",@"-3",@"1",@"-7", nil];
    fourArray=[[NSArray alloc] initWithObjects:@"-6",@"0",@"-4",@"2",@"-8", nil];
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    ...
    if (alertView.tag==2) {
        if (buttonIndex == 0) {
            // do something here...
            QuizNineViewController *quizViewCtrl=[[QuizNineViewController alloc] init];
