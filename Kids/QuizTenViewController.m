...
#import "QuizTenViewController.h"

@interface QuizTenViewController (){
...
@implementation QuizTenViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    order=0;
    problemArray=[[NSArray alloc] initWithObjects:@"2-2*3+2",@"6-4*2+3",@"4-3*3+5",@"3-2*4+3",@"6-1*2-3",nil];
    firstArray=[[NSArray alloc] initWithObjects:@"-3",@"1",@"-1",@"-3",@"1", nil];
    secondArray=[[NSArray alloc] initWithObjects:@"-2",@"2",@"0",@"-2",@"2", nil];
    thirdArray=[[NSArray alloc] initWithObjects:@"-1",@"3",@"1",@"-1",@"3", nil];
    fourArray=[[NSArray alloc] initWithObjects:@"0",@"4",@"2",@"0",@"4", nil];
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    ...
    if (alertView.tag==2) {
        if (buttonIndex == 0) {
            // do something here...
            QuizElevenViewController *quizViewCtrl=[[QuizElevenViewController alloc] init];
