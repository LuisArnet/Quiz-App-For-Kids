...
#import "QuizElevenViewController.h"

@interface QuizElevenViewController (){
...
@implementation QuizElevenViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    order=0;
    topproblemArray=[[NSArray alloc] initWithObjects:@"12+",@"16+",@"34+",@"15+",@"34+",nil];
    underproblemArray=[[NSArray alloc] initWithObjects:@"22",@"13",@"14",@"12",@"43", nil];
    firstArray=[[NSArray alloc] initWithObjects:@"33",@"26",@"46",@"27",@"77", nil];
    secondArray=[[NSArray alloc] initWithObjects:@"34",@"27",@"47",@"28",@"78", nil];
    thirdArray=[[NSArray alloc] initWithObjects:@"35",@"28",@"48",@"29",@"79", nil];
    fourArray=[[NSArray alloc] initWithObjects:@"36",@"29",@"49",@"30",@"80", nil];
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    ...
    if (alertView.tag==2) {
        if (buttonIndex == 0) {
            // do something here...
            QuizTwelveViewController *quizViewCtrl=[[QuizTwelveViewController alloc] init];
