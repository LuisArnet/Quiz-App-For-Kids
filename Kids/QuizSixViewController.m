...
#import "QuizSixViewController.h"

@interface QuizSixViewController (){
...
@implementation QuizSixViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    order=0;
    problemArray=[[NSArray alloc] initWithObjects:@"2-2*2",@"3-3*3",@"1-2*3",@"3-2*2",@"4-3*2",nil];
    firstArray=[[NSArray alloc] initWithObjects:@"-1",@"-6",@"-4",@"-1",@"-3", nil];
    secondArray=[[NSArray alloc] initWithObjects:@"-2",@"-7",@"-5",@"-2",@"-2", nil];
    thirdArray=[[NSArray alloc] initWithObjects:@"-3",@"-8",@"-6",@"-3",@"-1", nil];
    fourArray=[[NSArray alloc] initWithObjects:@"-4",@"-9",@"-7",@"-4",@"-4", nil];
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    ...
    if (alertView.tag==2) {
        if (buttonIndex == 0) {
            // do something here...
            QuizSevenViewController *quizViewCtrl=[[QuizSevenViewController alloc] init];
