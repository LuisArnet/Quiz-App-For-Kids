...
#import "QuizThreeViewController.h"

@interface QuizThreeViewController (){
...
@implementation QuizThreeViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    order=0;
    problemArray=[[NSArray alloc] initWithObjects:@"7-4",@"9-7",@"6-2",@"5-4",@"9-3",nil];
    firstArray=[[NSArray alloc] initWithObjects:@"1",@"2",@"3",@"1",@"6", nil];
    secondArray=[[NSArray alloc] initWithObjects:@"2",@"3",@"4",@"2",@"7", nil];
    thirdArray=[[NSArray alloc] initWithObjects:@"3",@"4",@"5",@"3",@"8", nil];
    fourArray=[[NSArray alloc] initWithObjects:@"4",@"5",@"6",@"4",@"9", nil];
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    ...
    if (alertView.tag==2) {
        if (buttonIndex == 0) {
            // do something here...
            QuizFourViewController *quizViewCtrl=[[QuizFourViewController alloc] init];
